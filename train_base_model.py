...
dataset_name=None):
    ...
    config = get_baseconfig_by_epoch(network_type=network_type, dataset_name=dataset_name,
                                     ...
                                     tb_dir=log_dir, save_weights=None, val_epoch_period=5,
                                     linear_final_lr=lrs.linear_final_lr,
                                     weight_decay_bias=weight_decay_bias, deps=deps)
    model=get_init_model(local_rank, config, show_variables=True, convbuilder=builder, use_nesterov=False,
...
if __name__ == '__main__':
    ...
    if network_type == 'src74':
        ...
        raise ValueError('???')
    # get_base_model(local_rank=start_arg.local_rank,
